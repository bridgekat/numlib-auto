...
- Two such records are kept here. `notes/lean-lessons.md` is what a sub-agent should read before writing any Lean: symptom, cause and fix for every trap that has cost someone more than a few minutes. `notes/refactor-queue.md` is the orchestrator's: work that is known to be wanted but changes existing declarations, so it must wait for a tree nobody else is writing to. Sub-agents report into it by reporting to the orchestrator; they do not edit it.
