...
the annihilator ideal `{p | p(A) v = 0}` ([Saad, *Iterative Methods*][saad2003iterative] §6.2, Prop
6.1–6.2; [Saad, *Large Eigenvalue Problems*][saad2011numerical] Prop 6.1–6.3;
[Choi][choi2006iterative] Def 2.1; [Meurant–Strakoš][meurant2006lanczos] §2.1).
...
generator of the same kind of ideal, with the same proofs. They are stated afresh because
Mathlib's are the annihilator of an *algebra element* `a : A` under `Polynomial.aeval a`, whereas
what a Krylov method annihilates is a *vector*, `p ↦ p(A) v`. The two would be one if Mathlib's
pair were generalized to the `𝕜[X]`-module `Module.AEval' A`, where `Krylov.annIdealVec A v` is
the annihilator of the cyclic submodule generated by `v`.
-/
...
/-- Saad, *Iterative Methods*, §6.2: `𝒦_m = {p(A) v | deg p < m}`, "the subspace of all vectors
which can be written as `x = p(A) v` with `p` of degree not exceeding `m - 1`". -/
...
/-- The grade vanishes only in the two degenerate cases: `v = 0`, and the junk value taken when
the cyclic subspace is infinite-dimensional. -/
