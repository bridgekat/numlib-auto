...
/-- The Chebyshev value appearing in Saad (6.123) is at least `1`, in particular positive. -/
private theorem one_le_eval_T_ratio (m : ℕ) : 1 ≤ (T ℝ m).eval ((lmax + lmin) / (lmax - lmin)) :=
  one_le_eval_T (one_le_ratio hl hll) m

/-- Saad's minimax value: the sup of `|shifted|` over `[lmin, lmax]`. -/
...
/-- The geometric form of the minimax value (Saad (6.128)). -/
