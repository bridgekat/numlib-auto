...
* the sparsity estimate, a Hölder bound on `M = A⁻¹ - A⁻¹ R`: a sufficiently accurate approximate
  inverse has a nonzero pattern containing the large entries of `A⁻¹`, so when those entries are
  all comparable `M` must be as dense as `A⁻¹`;
* the quadratic convergence of the self-preconditioned minimal-residual iteration, which is a
  ...
  `t`, then it is at most `‖R²‖ ≤ ‖R‖²`. That one-line argument, with the residual recurrence
  `R' = (1 - α) R + α R²`, is the entire content of Saad's Propositions 10.13 and 10.14, and it
  mentions neither matrices, nor the Frobenius norm, nor sparsity.
...
linear `T : F →L[ℝ] G` and a target `c : G`, the map `z ↦ ‖c - T z‖²` has derivative
`e ↦ -2 ⟪c - T m, T e⟫` at `m`, that is, gradient `-2 T† (c - T m)`. Saad's Proposition 10.9 is
the instance where `F` and `G` are the matrices with the Frobenius inner product, `T = (A * ·)`
and `c = 1`, whose gradient is `-2 Aᵀ (1 - A M)`. -/
...
/-- Saad Proposition 10.10: in a normed ring — in particular for matrices under any
...
/-- The two-sided scaling variant of Saad (10.54): row and column scalings by units do not change
the conclusion, so a small *scaled* residual is just as good. -/
...
/-- Saad Propositions 10.13 (10.60) and 10.14: if the new residual `r'` is no larger than
`(1 - t) r + t r²` for *every* real `t` — which is what a minimal-residual step in the direction
`M R` achieves, since `(1 - t) R + t R²` is the residual after a step of length `t`
(`Preconditioner.residual_selfPreconditionedStep`) — then `‖r'‖ ≤ ‖r²‖ ≤ ‖r‖²`. The proof is to
take `t = 1`. This is the whole of the quadratic-convergence claim, in both the global and the
column-oriented form; it says nothing about *whether* the iteration converges, only how fast it
does once it does. -/
...
/-- The estimate behind Saad Proposition 10.11: with `B` a left inverse of `A` and
`R = 1 - A M` the residual, `M = B - B R`, so each entry of `M` differs from the corresponding
entry of `A⁻¹` by at most `(sup over the row of B) * (column sum of |R|)`. -/
...
/-- Saad Corollary 10.12: if the `j`-th column of the residual is small in the `ℓ¹` norm and the
entry `B i j` of the inverse is large relative to its own row, then `M i j` cannot vanish. So a
sufficiently accurate approximate inverse must have a nonzero pattern containing the large entries
of `A⁻¹`, and if those are all comparable — as they are for many matrices — `M` must be as dense
as `A⁻¹`. -/
