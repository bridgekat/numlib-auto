...
* `DomainDecomposition.isMinRes_map_iff` and `DomainDecomposition.isGalerkin_map_iff`: if moreover
...
on the reduced system: `DomainDecomposition.isMinRes_iff_isMinRes_schurComplement` and its Galerkin
twin as an equivalence, and `DomainDecomposition.exists_isMinRes_schurComplement` in the direction
...
theorem isMinRes_map_iff (hP : ∀ a b : Y, inner 𝕜 (P a) (P b) = (inner 𝕜 a b : 𝕜))
    (h : T ∘ₗ P = P ∘ₗ S) (hres : b - T u₀ = P (g - S y₀)) (K : Submodule 𝕜 Y) (y : Y) :
    IsMinRes T b u₀ (K.map P) (u₀ + P (y - y₀)) ↔ IsMinRes S g y₀ K y := by
...
`DomainDecomposition.isMinRes_map_iff`. -/
...
system; `DomainDecomposition.isMinRes_map_iff` read from the other side. -/
theorem exists_isMinRes_of_isMinRes_map
    (hP : ∀ a b : Y, inner 𝕜 (P a) (P b) = (inner 𝕜 a b : 𝕜)) (h : T ∘ₗ P = P ∘ₗ S)
    (hres : b - T u₀ = P (g - S y₀)) {K : Submodule 𝕜 Y} {u : X}
    (hu : IsMinRes T b u₀ (K.map P) u) : ∃ y, IsMinRes S g y₀ K y ∧ u = u₀ + P (y - y₀) := by
  obtain ⟨w, hw, rfl⟩ := exists_mem_of_sub_mem_map hu.mem
  refine ⟨y₀ + w, ?_, by rw [add_sub_cancel_left]⟩
  rw [← isMinRes_map_iff hP h hres K (y₀ + w), add_sub_cancel_left]
  exact hu
...
`DomainDecomposition.isMinRes_map_iff` to the Krylov subspaces generated by the initial residuals,
which is the form in which a Krylov method is specified. -/
theorem isMinResIterate_map_iff (hP : ∀ a b : Y, inner 𝕜 (P a) (P b) = (inner 𝕜 a b : 𝕜))
    (h : T ∘ₗ P = P ∘ₗ S) (hres : b - T u₀ = P (g - S y₀)) (k : ℕ) (y : Y) :
    Krylov.IsMinResIterate T b u₀ k (u₀ + P (y - y₀)) ↔ Krylov.IsMinResIterate S g y₀ k y := by
  have hK := krylov_subspace_residual_eq_map h hres k
  constructor
  · intro hx
    have hx' : IsMinRes T b u₀ (Krylov.subspace T (b - T u₀) k) (u₀ + P (y - y₀)) := hx
    rw [hK] at hx'
    exact (isMinRes_map_iff hP h hres _ y).1 hx'
  · intro hy
    have hy' : IsMinRes S g y₀ (Krylov.subspace S (g - S y₀) k) y := hy
    have hx' := (isMinRes_map_iff hP h hres _ y).2 hy'
    rw [← hK] at hx'
    exact hx'

/-- **The Galerkin Krylov iterates of the two systems correspond**, the twin of
`DomainDecomposition.isMinResIterate_map_iff`. -/
...
`y` the corresponding iterate of the small system; `DomainDecomposition.isMinResIterate_map_iff`
read from the other side. -/
theorem exists_isMinResIterate_of_isMinResIterate_map
    (hP : ∀ a b : Y, inner 𝕜 (P a) (P b) = (inner 𝕜 a b : 𝕜)) (h : T ∘ₗ P = P ∘ₗ S)
    (hres : b - T u₀ = P (g - S y₀)) (k : ℕ) {u : X} (hu : Krylov.IsMinResIterate T b u₀ k u) :
    ∃ y, Krylov.IsMinResIterate S g y₀ k y ∧ u = u₀ + P (y - y₀) := by
  have hu' : IsMinRes T b u₀ (Krylov.subspace T (b - T u₀) k) u := hu
  rw [krylov_subspace_residual_eq_map h hres k] at hu'
  obtain ⟨y, hy, hyu⟩ := exists_isMinRes_of_isMinRes_map hP h hres hu'
  exact ⟨y, hy, hyu⟩
...
theorem isMinRes_iff_isMinRes_schurComplement (hB : IsUnit B) (hLS : IsUnit LS) (hUS : IsUnit US)
    (f : m → 𝕜) (g y₀ y : n → 𝕜) (k : ℕ) :
    Krylov.IsMinResIterate (Matrix.toEuclideanLin (precondMatrix B E F C LS US))
        (WithLp.toLp 2 ((blockLower B F LS)⁻¹ *ᵥ Sum.elim f g))
        (WithLp.toLp 2 (blockUpper B E US *ᵥ consistentGuess B E f y₀)) k
        (WithLp.toLp 2 (blockUpper B E US *ᵥ consistentGuess B E f y))
      ↔ Krylov.IsMinResIterate
        (Matrix.toEuclideanLin (precondSchurComplement B E F C LS US))
  ...
  exact isMinResIterate_map_iff (inner_prolong_prolong 𝕜 m n) hcomm
...
theorem exists_isMinRes_schurComplement (hB : IsUnit B) (hLS : IsUnit LS) (hUS : IsUnit US)
    (f : m → 𝕜) (g y₀ : n → 𝕜) (k : ℕ) {u : EuclideanSpace 𝕜 (m ⊕ n)}
    (hu : Krylov.IsMinResIterate (Matrix.toEuclideanLin (precondMatrix B E F C LS US))
      (WithLp.toLp 2 ((blockLower B F LS)⁻¹ *ᵥ Sum.elim f g))
      (WithLp.toLp 2 (blockUpper B E US *ᵥ consistentGuess B E f y₀)) k u) :
    ∃ y : n → 𝕜,
      Krylov.IsMinResIterate (Matrix.toEuclideanLin (precondSchurComplement B E F C LS US))
          (WithLp.toLp 2 (LS⁻¹ *ᵥ reducedRhs B F f g)) (WithLp.toLp 2 (US *ᵥ y₀)) k
          (WithLp.toLp 2 (US *ᵥ y))
        ∧ u = WithLp.toLp 2 (blockUpper B E US *ᵥ consistentGuess B E f y) := by
  have hcomm := precondMatrix_comp_prolong (E := E) (F := F) (C := C) hB hLS hUS
  obtain ⟨w, hw, hwu⟩ := exists_isMinResIterate_of_isMinResIterate_map
...
/-- **The Galerkin twin of `DomainDecomposition.isMinRes_iff_isMinRes_schurComplement`**: the same
