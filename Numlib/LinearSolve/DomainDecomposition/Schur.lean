...
* `DomainDecomposition.isMinResidual_map_iff` and `DomainDecomposition.isGalerkin_map_iff`: if
  moreover `P` preserves inner products and the initial residual of the big system is `P` of that of
  the small one, then the minimal-residual (respectively Galerkin) iterates of the two systems
...
on the reduced system: `DomainDecomposition.isMinResidual_iff_isMinResidual_schurComplement` and its
Galerkin twin as an equivalence, and `DomainDecomposition.exists_isMinResidual_schurComplement` in
the direction that *every* iterate of the full system arises this way.
...
theorem exists_isMinResidual_of_isMinResidual_map
    ...
    (hu : IsMinResidual T b u₀ (K.map P) u) :
    ∃ y, IsMinResidual S g y₀ K y ∧ u = u₀ + P (y - y₀) := by
...
`DomainDecomposition.isMinResidual_map_iff` to the Krylov subspaces generated by the initial
residuals, which is the form in which a Krylov method is specified. -/
theorem isMinResidualIterate_map_iff (hP : ∀ a b : Y, inner 𝕜 (P a) (P b) = (inner 𝕜 a b : 𝕜))
    (h : T ∘ₗ P = P ∘ₗ S) (hres : b - T u₀ = P (g - S y₀)) (k : ℕ) (y : Y) :
    Krylov.IsMinResidualIterate T b u₀ k (u₀ + P (y - y₀)) ↔
      Krylov.IsMinResidualIterate S g y₀ k y := by
...
`y` the corresponding iterate of the small system;
`DomainDecomposition.isMinResidualIterate_map_iff` read from the other side. -/
...
theorem isMinResidual_iff_isMinResidual_schurComplement (hB : IsUnit B) (hLS : IsUnit LS)
...
/-- **The Galerkin twin of `DomainDecomposition.isMinResidual_iff_isMinResidual_schurComplement`**:
the same correspondence for the orthogonal-projection specification, so that [saad2003iterative],
