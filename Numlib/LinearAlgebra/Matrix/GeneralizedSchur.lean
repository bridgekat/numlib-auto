...
`Matrix.exists_unitary_pencil_isUpperTriangular` of `Numlib/Eigen/Pencil`; it is not written out.
...
/-- **The orthogonal group is compact**: closed, and contained in the cube of matrices with
entries in `[-1, 1]`. -/
theorem isCompact_orthogonalGroup :
    IsCompact ((orthogonalGroup (Fin n) ℝ : Submonoid _) : Set (Matrix (Fin n) (Fin n) ℝ)) := by
  have hcube : IsCompact (Set.pi Set.univ fun _ : Fin n => Set.pi Set.univ fun _ : Fin n =>
      Set.Icc (-1 : ℝ) 1) :=
    isCompact_univ_pi fun _ => isCompact_univ_pi fun _ => isCompact_Icc
  refine hcube.of_isClosed_subset isClosed_orthogonalGroup fun U hU => ?_
  simp only [Set.mem_pi, Set.mem_univ, forall_const, Set.mem_Icc]
  intro i j
  exact abs_le.1 (abs_apply_le_one_of_mem_orthogonalGroup hU i j)

/-! ### The general case, by perturbation -/

/-- `B + t I` is nonsingular for all small `t > 0`: `det (B + t I)` is the characteristic
polynomial of `-B` at `t`, which has finitely many roots. -/
theorem exists_forall_isUnit_det_add_smul_one (B : Matrix (Fin n) (Fin n) ℝ) :
    ∃ δ > 0, ∀ t : ℝ, 0 < t → t < δ → IsUnit (B + t • (1 : Matrix (Fin n) (Fin n) ℝ)).det := by
  classical
  have hdet : ∀ t : ℝ, (B + t • 1).det = (-B).charpoly.eval t := by
    intro t
  ...
  set S := ((-B).charpoly.roots.toFinset).filter fun t => 0 < t with hS
  ...
  · rw [isUnit_iff_ne_zero, hdet]
    intro hroot
    have htS : t ∈ S := by
      rw [hS, Finset.mem_filter, Multiset.mem_toFinset, Polynomial.mem_roots hne]
      exact ⟨hroot, ht0⟩
