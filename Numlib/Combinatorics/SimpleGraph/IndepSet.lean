...
large: every vertex outside it has a neighbour inside it, so the whole vertex type is covered by the
set together with the neighbourhoods of its members, and counting gives `Fintype.card V ≤
(G.maxDegree + 1) * s.card` (`SimpleGraph.card_le_mul_card_of_maximal_isIndepSet`), that is `|s| ≥
|V| / (1 + Δ)`. The degree bound is only ever used on the members of `s`, so the sharper
...
[saad2003iterative] §3.3.3: an independent set of size `s` gives a diagonal leading block of size
`s` in the reordered matrix, so the bound says how much of the matrix one elimination step can
clear. [saad2003iterative] states it for the set his greedy algorithm produces; the argument he
gives needs no algorithm, only maximality, and that is how it is stated here.
-/
...
/-- A maximal independent set *dominates*: every vertex outside it has a neighbour inside it, since
...
/-- The size of a maximal independent set, with the degree bound taken over the members of the set
only: if every `w ∈ s` has `G.degree w ≤ ν`, then `Fintype.card V ≤ (ν + 1) * s.card`.
...
/-- [saad2003iterative] bound on the size of a maximal independent set: for a maximal independent
set `s` of a finite simple graph, `Fintype.card V ≤ (G.maxDegree + 1) * s.card`, that is `|s| ≥ |V|
/ (1 + Δ)`. -/
