...
([saad2003iterative], §12.3). Two of the three ingredients are here; the two ways of choosing `s`
optimally are `Numlib/Preconditioner/Chebyshev` (uniform norm on an enclosing interval)
and `Numlib/RingTheory/Polynomial/KernelPolynomial` (weighted least squares).

Nothing here is analytic.

* The Neumann polynomial `∑_{i ≤ s} (1 - ω X)^i` and the identity behind it are a geometric sum in a
  ring, stated for `1 - m * a` with an arbitrary ring element `m` so that the block-diagonal `D` of
  [saad2003iterative] general form `M⁻¹ = (∑_{i ≤ s} N^i) D⁻¹`, `N = 1 - ω D⁻¹ A`, costs nothing
  extra. Since `(∑_{i ≤ s} N^i) (m a) = 1 - N^{s+1}`, every bound on the preconditioned operator is
  a bound on `‖N^{s+1}‖`: a Neumann preconditioner is `s + 1` steps of the underlying stationary
  iteration of `Numlib/Stationary/Basic`.
* The optimality criterion is `Numlib/Krylov/Convergence/Polynomial` read backwards: what a
  polynomial preconditioner controls is `sup_{[α,β]} |1 - t s(t)|`, and that bounds `‖x - s(A) (A
  x)‖` by the compression trick, in any inner product space and with no functional calculus.
...
/-- The Neumann polynomial `∑_{i ≤ s} (1 - ω X)^i` of [saad2003iterative] §12.3.1: the polynomial
`p` for which `p(A)` is the `s`-term truncation of the Neumann series for `(ω A)⁻¹`. -/
...
/-- [saad2003iterative] (12.3): with `n = 1 - m * a` the truncated Neumann series satisfies `(∑_{i ≤
s} n^i) (m a) = 1 - n^(s+1)` in any ring, so the preconditioned operator `M⁻¹ A` of a Neumann
preconditioner differs from the identity by exactly the `(s+1)`-st power of the iteration operator
of the underlying splitting. -/
...
operator is not symmetric for the ambient inner product, and CG only ever needs symmetry for the one
it actually uses. -/
...
/-- [saad2003iterative] Exercise 12.1, and the reason polynomial preconditioning is compatible with
CG: if `B` is self-adjoint for the inner product of a symmetric coercive `D` — for `B = D⁻¹ A` with
`A` symmetric, by `Preconditioner.energyInner_comm_of_comp_eq` — then the preconditioned operator
`s(B) B` is symmetric for that same inner product, for every real polynomial `s`. With `D = 1` this
is the statement that `s(A) A` is symmetric. -/
...
/-- The optimality criterion of [saad2003iterative] (12.4)–(12.5) as a bound: for a symmetric `A`
whose quadratic form lies in `[α, β]` and a real polynomial `s`, `‖x - s(A) (A x)‖ ≤ (sup_{t ∈ [α,
β]} |1 - t s(t)|) ‖x‖` in any inner product space. Choosing `s` to make `sup |1 - t s(t)|` small is
therefore exactly what makes the preconditioned operator `s(A) A` close to the identity. The version
over the spectrum rather than an enclosing interval is `LinearMap.IsSymmetric.norm_aeval_apply_le`.
-/
