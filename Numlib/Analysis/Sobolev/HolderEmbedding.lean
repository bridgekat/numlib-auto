...
* `SobolevMultiIndex.exists_succ_of_partialDeriv`, `MultiIndexLE.exists_eq_addSingle`: the
  inductive step of the order lowering — an element of `W^{k+1,p}` whose function and partial
  derivatives are (a.e.) functions of elements of `W^{j+1,q}` is the function of an element of
  `W^{j+2,q}`, with a norm bound.
* `SobolevEuclidean.exists_continuousLinearMap_of_forall_eLpNorm_fn_le`: **lowering the order
  to `j + 1`** — from an `L^q` bound on `W^{m,p}(Ω)`, a bounded linear
  `W^{m+j+1,p}(Ω) → W^{j+1,q}(Ω)` preserving the function; the generalization of the order-one
  map `SobolevEuclidean.exists_continuousLinearMap_orderOne` of
  `Numlib/Analysis/Sobolev/DenyLions.lean`.
...
/-! ### Lowering the order: `W^{m+j+1,p}(Ω) → W^{j+1,q}(Ω)` -/
...
order at most `k`. -/
theorem MultiIndexLE.exists_eq_addSingle [DecidableEq ι] {k : ℕ} (β : MultiIndexLE ι (k + 1))
...
variable {E F : Type*} [NormedAddCommGroup E] [NormedSpace ℝ E] [MeasurableSpace E]
  [OpensMeasurableSpace E] [NormedAddCommGroup F] [NormedSpace ℝ F] [LinearOrder ι]
  {b : Basis ι ℝ E} {Ω : Opens E} {μ : Measure E} [IsLocallyFiniteMeasure μ]
  [FiniteDimensional ℝ E] [BorelSpace E] [CompleteSpace F] {p q : ℝ≥0∞} [Fact (1 ≤ q)]
