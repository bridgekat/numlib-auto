...
* `SobolevEuclidean.isCompactEmbedding_toLowerOrderL_of_isSobolevExtensionDomain`:
  ...
  and every dimension (`[han2009theoretical]` Theorem 7.3.9 under the extension hypothesis;
  the `C¹` chart form is `SobolevEuclidean.isCompactEmbedding_toLower_of_lt`).
...
* `SobolevEuclidean.exists_continuousLinearMap_orderOne`: the inclusion `W^{m+1,p}(Ω) ⊆ W^{1,r}(Ω)`
  of an extension domain as a bounded linear map, for the exponents `r` of Corollary 9.15 of
  Brezis; with it, **the Rellich–Kondrachov theorem at every order** on an extension domain of
  finite measure — `SobolevEuclidean.isCompactEmbedding_toLpₗ_of_order_of_lt` (`k < N/p`,
  `q < p*`), `…_of_order_of_eq` (`k = N/p`, every `q < ∞`),
  `SobolevEuclidean.exists_isCompactEmbedding_toContinuousMap_of_order` (`k > N/p`, into `C(Ω̄)`)
  and `SobolevEuclidean.isCompactEmbedding_toLowerOrderL_of_lt` (`W^{k,p} ⊂⊂ W^{l,p}`, `l < k`),
  which are Atkinson–Han's Theorems 7.3.8 and 7.3.9 under the extension hypothesis.
...
/-! ### Lowering the order: `W^{m+1,p}(Ω) → W^{1,r}(Ω)` as a bounded linear map -/
...
map**, for `1 ≤ p ≤ r < ∞` with `1/p − m/N ≤ 1/r` (and `N ≥ 2` or `p > 1`): the element of
`W^{1,r}(Ω)` with the same function that
`SobolevEuclidean.exists_sobolevEuclidean_one_of_forall_eLpNorm_fn_restrict_le` produces from
the `L^r` bound of `[brezis2011functional]` Corollary 9.15 at order `m`; it is unique
(`SobolevMultiIndex.ext_of_fn_ae_eq`), hence linear, and bounded. This is the device by which
the embeddings of `W^{m+1,p}(Ω)` are read off those of `W^{1,r}(Ω)`. -/
theorem SobolevEuclidean.exists_continuousLinearMap_orderOne (m : ℕ) {p r : ℝ≥0}
  ...
  choose w hw hwn using fun u : SobolevEuclidean N (m + 1) p Ω ↦
    SobolevEuclidean.exists_sobolevEuclidean_one_of_forall_eLpNorm_fn_restrict_le hK hKu u
  refine ⟨LinearMap.mkContinuous
    { toFun := w
      map_add' := fun u v ↦ ext_of_fn_ae_eq ?_
      map_smul' := fun c u ↦ ext_of_fn_ae_eq ?_ } ((N + 1) * K.toReal) fun u ↦ hwn u, hw⟩
  · exact (hw _).trans ((fn_add u v).trans (((hw u).symm.add (hw v).symm).trans (fn_add _ _).symm))
...
theorem exists_fn_ae_eq_of_contDiff
