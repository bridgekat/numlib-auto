...
* `gradientRange_eq_domSubdifferential_conj`, `relint_dom_conj_subset_gradientRange`,
  `gradientRange_subset_dom_conj` — `D = dom ∂f*`, therefore `ri (dom f*) ⊆ D ⊆ dom f*`.
* `legendreType_conj_iff`, `bijOn_gradient_of_legendreType`, `continuousOn_gradient_interior_dom` —
  the Legendre duality ([rockafellar1970convex] Theorem 26.5).
...
theorem hasGradientAtFn_toDual_iff_mem_subdifferential (hf : ConvexFn f) (hp : Proper f)
...
theorem gradientRange_eq_domSubdifferential_conj (hf : ConvexFn f) (hp : Proper f)
    (hcl : ClosedFn f) (hes : EssentiallySmooth f) :
    gradientRange f = domSubdifferential (innerₗ E) (conj (innerₗ E) f) := by
  ext w
  ...
  exact (mem_subdifferential_conj_innerL_iff hf hcl z w).symm
...
theorem relint_dom_conj_subset_gradientRange (hf : ConvexFn f) (hp : Proper f) (hcl : ClosedFn f)
  ...
  rw [gradientRange_eq_domSubdifferential_conj hf hp hcl hes]
  exact fun w hw =>
    subdifferential_nonempty_of_mem_relint_dom (convexFn_conj _ f) (proper_conj ⟨hf, hcl, hp⟩) hw
...
theorem gradientRange_subset_dom_conj (hf : ConvexFn f) (hp : Proper f) (hcl : ClosedFn f)
  ...
  rw [gradientRange_eq_domSubdifferential_conj hf hp hcl hes]
  exact domSubdifferential_subset_dom (proper_conj ⟨hf, hcl, hp⟩)
...
theorem strictConvexOnFn_conj_of_subset_gradientRange (hf : ConvexFn f) (hp : Proper f)
  ...
  (essentiallyStrictlyConvex_conj_iff_essentiallySmooth hf hp hcl).2 hes hC
    (by rw [← gradientRange_eq_domSubdifferential_conj hf hp hcl hes]; exact hCsub)
...
theorem legendreType_iff_subdifferential_injective (hf : ConvexFn f) (hp : Proper f)
    (hcl : ClosedFn f) :
    LegendreType f ↔
...
theorem legendreType_conj_iff (hf : ConvexFn f) (hp : Proper f) (hcl : ClosedFn f) :
  ...
  rw [legendreType_iff_subdifferential_injective (convexFn_conj _ f) (proper_conj ⟨hf, hcl, hp⟩)
      closedFn_conj,
    legendreType_iff_subdifferential_injective hf hp hcl,
    subsingleton_subdifferential_conj_iff hf hcl, pairwise_disjoint_subdifferential_conj_iff hf hcl,
    and_comm]

theorem LegendreType.conj (hf : ConvexFn f) (hp : Proper f) (hcl : ClosedFn f)
    (hleg : LegendreType f) : LegendreType (ConvexAnalysis.conj (innerₗ E) f) :=
...
theorem hasGradientAtFn_conj_iff (hf : ConvexFn f) (hp : Proper f) (hcl : ClosedFn f)
  ...
  rw [hasGradientAtFn_toDual_iff_mem_subdifferential hf hp hcl hleg.1,
    hasGradientAtFn_toDual_iff_mem_subdifferential (convexFn_conj _ f) (proper_conj ⟨hf, hcl, hp⟩)
      closedFn_conj (hleg.conj hf hp hcl).1,
    mem_subdifferential_conj_innerL_iff hf hcl]
...
theorem gradientRange_eq_interior_dom_conj (hf : ConvexFn f) (hp : Proper f) (hcl : ClosedFn f)
  ...
  rw [gradientRange_eq_domSubdifferential_conj hf hp hcl hleg.1,
    domSubdifferential_eq_interior_dom_of_essentiallySmooth (convexFn_conj _ f)
