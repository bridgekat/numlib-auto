...
* `epsSubdifferential_eq_setOf_conj_le`, `iInter_epsSubdifferential` — `∂_ε f x` is a level set of a
...
the level is `0`. -/
theorem epsSubdifferential_eq_setOf_conj_le (hr : f x = (r : EReal)) (ε : ℝ) :
    epsSubdifferential B ε f x = {y : F | conj B (shiftFn f x (ε - r)) y ≤ 0} := by
  rw [epsSubdifferential_eq_supportSet hr ε, supportSet_eq_setOf_conj_le]
...
theorem supportFn_epsSubdifferential [IsCompatiblePairing B] (hf : ConvexFn f) (hp : Proper f)
    (hc : IsClosed (epi f)) (hr : f x = (r : EReal)) (hε : 0 < ε) :
    supportFn B.flip (epsSubdifferential B ε f x) = posHomGen (shiftFn f x (ε - r)) := by
  rw [epsSubdifferential_eq_setOf_conj_le hr ε]
...
theorem supportFn_epsSubdifferential_apply [IsCompatiblePairing B] (hf : ConvexFn f) (hp : Proper f)
