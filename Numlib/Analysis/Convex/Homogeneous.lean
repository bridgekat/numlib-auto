...
* `convex_iff_add_mem_of_isCone` — a cone is convex iff it is closed under addition. This is
  Mathlib's `ConvexCone` content, repackaged as a statement about bare sets.
* `span_eq_sub_of_isCone` — the subspace generated by a convex cone containing the origin is
  `s - s`; with `vectorSpan_eq_span_of_zero_mem`, so is its affine hull.
* `smul_closure_eq_of_isCone` — the closure of a cone is a cone. The one topological
  statement in the file; it needs only that each individual scaling is continuous.
* `PosHomogeneous.convexFn_iff_subadditive` — for a positively homogeneous `f`, convexity is
  subadditivity.
* `PosHomogeneous.sum_le`, `PosHomogeneous.neg_le` — subadditivity over a finite positive
  combination, and `-(f x) ≤ f (-x)`.
* `PosHomogeneous.isLinearOn_iff`, `PosHomogeneous.exists_linearMap_iff` —
  linearity on a subspace `L` is equivalent to `f (-x) = -(f x)` on `L`, and
  `PosHomogeneous.neg_eq_of_mem_span` reduces the check to a spanning set, in particular a basis.
...
theorem span_eq_sub_of_isCone (hs : ∀ a : ℝ, 0 < a → a • s = s) (hconv : Convex ℝ s)
    (h0 : (0 : M) ∈ s) : (Submodule.span ℝ s : Set M) = s - s := by
  have hadd := (convex_iff_add_mem_of_isCone hs).1 hconv
  have hsmul : ∀ a : ℝ, 0 < a → ∀ x ∈ s, a • x ∈ s := fun a ha x hx =>
    (smul_mem_iff_of_isCone hs ha).2 hx
  ...
  · induction hz using Submodule.span_induction with
    ...
    | smul c u _ hu =>
        ...
        · exact ⟨(-c) • y, hsmul _ (by linarith) y hy, (-c) • x, hsmul _ (by linarith) x hx,
            by module⟩
        · subst hc
          exact ⟨0, h0, 0, h0, by simp⟩
        · exact ⟨c • x, hsmul c hc x hx, c • y, hsmul c hc y hy, by module⟩
  · rintro _ ⟨x, hx, y, hy, rfl⟩
    exact (Submodule.span ℝ s).sub_mem (Submodule.subset_span hx) (Submodule.subset_span hy)
...
variable {M : Type*} [AddCommGroup M] [Module ℝ M] [TopologicalSpace M]
  [ContinuousConstSMul ℝ M] {s : Set M}
...
theorem smul_closure_eq_of_isCone (hs : ∀ a : ℝ, 0 < a → a • s = s) (a : ℝ) (ha : 0 < a) :
    a • closure s = closure s := by
  rw [← closure_smul₀' ha.ne' s, hs a ha]
