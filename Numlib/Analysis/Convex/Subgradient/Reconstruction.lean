...
* `recessionCone_subgradient_eq_normalCone` — the recession cone of `∂f x` is `N_{dom f}(x)`.
...
`recessionCone_subgradient_eq_normalCone` is proved on its own and used nowhere below.
