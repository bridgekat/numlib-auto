...
* `scalingSystem φ j k` is `2 ^ (j / 2) φ (2 ^ j x - k)`, the dilate by `2 ^ j` of the translate of
  ...
  `1` with the identity.
* `IsMultiresolutionAnalysis V φ` is [han2009theoretical] Definition 4.5.1 [han2009theoretical]. Two
  of its clauses depart from the book's phrasing, and deliberately. *Shift invariance* is stated as
  orthonormality of the translates together with `V 0` being the closed span of them, rather than as
  "the translates are an orthonormal basis of `V 0`": the two are equivalent, the former is what a
...
[han2009theoretical] (4.5.4) and Daubechies' compactly supported families are not developed: the
book states them without proof, and the standard arguments need conditions on `φ̂` that Mathlib has
no vocabulary for.
-/
