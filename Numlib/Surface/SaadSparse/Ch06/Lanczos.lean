...
The structural result of the section is **Theorem 6.19** (`thm_6_19`) together with the
...
theorem thm_6_19 (hA : A.IsSymm) (hv : ‖v₁‖ = 1) (m : ℕ) :
...
`β_{j+1} = h_{j+1,j}`. -/
theorem alg_6_15_eq_alg_6_2 (hA : A.IsSymm) (hv : ‖v₁‖ = 1) (j : ℕ) :
...
theorem eq_6_85_isomorphism {m : ℕ} (hm : m ≤ grade A v₁) :
...
grade of `v_1`. -/
theorem eq_6_85_nondegenerate {m : ℕ} (hm : m ≤ grade A v₁) {p : ℝ[X]} (hp : p.degree < m)
...
theorem eq_6_85_orthogonal_polynomials (hA : A.IsSymm) (hv : ‖v₁‖ = 1) {i : ℕ}
    (hi : i < grade A v₁) :
