...
problem is **unisolvent** when it has exactly one solution for every datum.

* `Approximation.interpMap Vₙ L` is the linear map `u ↦ (L i u)` from `Vₙ` to `Fin n → 𝕜`, and
  `Approximation.IsUnisolvent Vₙ L` says that it is bijective
...
Analysis Framework*, 3rd edition, Springer, 2009, Definition 3.2.1 and Theorem 3.2.3. -/
...
theorem isUnisolvent_iff_isUnit :
  ...
  classical
  set M : Matrix (Fin n) (Fin n) 𝕜 := Matrix.of fun i j => L i (v j : V) with hM
  have hcomp : ⇑(interpMap Vₙ L) = M.mulVec ∘ v.equivFun :=
    funext fun u => interpMap_eq_mulVec v u
  rw [isUnisolvent_iff_bijective, hcomp, Function.Bijective.of_comp_iff _ v.equivFun.bijective]
  exact ⟨fun h => Matrix.mulVec_injective_iff_isUnit.mp h.1,
    fun h => ⟨Matrix.mulVec_injective_iff_isUnit.mpr h,
...
Reference: Kendall Atkinson and Weimin Han, *Theoretical Numerical Analysis: A Functional
Analysis Framework*, 3rd edition, Springer, 2009, Lemma 3.2.2. -/
...
Reference: Kendall Atkinson and Weimin Han, *Theoretical Numerical Analysis: A Functional
Analysis Framework*, 3rd edition, Springer, 2009, Theorem 3.2.3. -/
theorem isUnisolvent_iff_forall_eq_zero :
  ...
  have hcomp : ⇑(interpMap Vₙ L) = M.mulVec ∘ v.equivFun :=
    funext fun u => interpMap_eq_mulVec v u
  have hinj : Function.Injective (interpMap Vₙ L) ↔ IsUnit M := by
    rw [hcomp, Function.Injective.of_comp_iff' _ v.equivFun.bijective,
      Matrix.mulVec_injective_iff_isUnit]
  rw [isUnisolvent_iff_isUnit v, ← hinj, injective_iff_map_eq_zero (interpMap Vₙ L)]
...
theorem isUnisolvent_iff_forall_exists :
  ...
  have hcomp : ⇑(interpMap Vₙ L) = M.mulVec ∘ v.equivFun :=
    funext fun u => interpMap_eq_mulVec v u
  have hsurj : Function.Surjective (interpMap Vₙ L) ↔ IsUnit M := by
    rw [hcomp, Function.Surjective.of_comp_iff _ v.equivFun.surjective,
      Matrix.mulVec_surjective_iff_isUnit]
  rw [isUnisolvent_iff_isUnit v, ← hsurj]
...
Reference: Kendall Atkinson and Weimin Han, *Theoretical Numerical Analysis: A Functional
Analysis Framework*, 3rd edition, Springer, 2009, (3.2.3). -/
...
Reference: Kendall Atkinson and Weimin Han, *Theoretical Numerical Analysis: A Functional
Analysis Framework*, 3rd edition, Springer, 2009, §3.3.4. -/
theorem haarCondition_iff_isUnisolvent {X : Type*} [TopologicalSpace X] [CompactSpace X]
  ...
  classical
  set v := Module.finBasisOfFinrankEq ℝ (Vₙ : Submodule ℝ C(X, ℝ)) hdim with hv
  constructor
  · intro hhaar x hx
  ...
  · intro huni g hg hg0 s hs
    ...
    set x : Fin n → X := fun i => ((e.symm i : t) : X) with hxdef
