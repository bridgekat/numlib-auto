...
boundedness with constant `M`, coercivity `c ‖v‖² ≤ re (a v v)` (what [han2009theoretical] call
"`V`-elliptic" or "strongly positive"), Hermitian symmetry, the associated operator
...
/-- `c ‖v‖² ≤ re (a v v)`. [han2009theoretical], call such a form `V`-elliptic with constant `c`, or
strongly positive. -/
...
/-- The operator norm is the *least* boundedness constant: any nonnegative `M` with `‖a u v‖ ≤ M ‖u‖
‖v‖` dominates `‖a‖`.  With `isBoundedWith_opNorm` this identifies `‖a‖` with the smallest constant
in the definition of boundedness. -/
theorem opNorm_le_of_isBoundedWith {M : ℝ} (hM : 0 ≤ M) (h : a.IsBoundedWith M) : ‖a‖ ≤ M :=
  a.opNorm_le_bound₂ hM h

/-- Coercivity only weakens as its constant shrinks: a form coercive with constant `c` is coercive
with every `c' ≤ c`. -/
...
/-- Over real scalars `re` is the identity, so coercivity is the bare inequality `c ‖v‖² ≤ a v v`.
-/
...
/-- The operator `A` with `⟪A u, v⟫ = a u v`, obtained by applying the Riesz representation theorem
to each of the functionals `a u` ([han2009theoretical], (8.3.1) and (9.4.5)); Mathlib's
...
/-- The defining property of `SesqForm.toOperator`: `⟪A u, v⟫ = a u v`.  Every fact about a form is
transported to its operator, and back, through this identity. -/
...
/-- An operator is recovered from its form.  Together with `ofOperator_toOperator`, `ofOperator` and
`toOperator` are mutually inverse: on a Hilbert space the bounded sesquilinear forms are exactly the
forms `⟪A u, v⟫` of bounded operators, with no repetition. -/
...
/-- Coercivity is the same condition on a form and on its operator: the two quadratic forms `re (a v
v)` and `re ⟪A v, v⟫` are literally the same function of `v`. -/
...
/-- A form is Hermitian exactly when its operator is symmetric, `⟪A u, v⟫ = ⟪u, A v⟫`; since `A` is
bounded and everywhere defined, that is self-adjointness. -/
...
representative of the functional `a u` and Riesz representation preserves norms.  So a bound `M` for
the form is a bound for `A`, and conversely. -/
...
/-- The energy functional `E(v) = ½ re (a v v) - re (ℓ v)`.  When `a` is Hermitian and coercive, its
minimizers are exactly the solutions of `a u v = ℓ v` ([han2009theoretical], Thm 8.3.3 and (9.1.7));
see `SesqForm.isMinOn_energy_iff`. -/
...
everything proved about energy norms of operators — Cauchy–Schwarz, the energy inner product and its
orthogonal projections — applies verbatim to a form. -/
...
energy norm is equivalent to the ambient norm ([han2009theoretical], Thm 8.3.3 and §9.4).  The upper
half is `energyNorm_le_sqrt_mul_norm`. -/
...
/-- Upper half of the norm equivalence `√c ‖v‖ ≤ ‖v‖_a ≤ √M ‖v‖`: a bound `M` on the form bounds the
energy norm by `√M ‖v‖`.  The lower half is `sqrt_mul_norm_le_energyNorm`. -/
...
/-- The inf–sup (Babuška–Brezzi) condition with constant `α`, in operator-norm form: `α ‖u‖ ≤ ‖a u‖
= sup_{v ≠ 0} |a u v| / ‖v‖` ([han2009theoretical], (8.7.2) and (9.2.3)).  For a one-space form it
is weaker than coercivity; together with `IsNondegenerate` it is what makes the two-space problem `a
u v = ℓ v` uniquely solvable, see `SesqForm₂.babuska_necas`. -/
def InfSupWith (α : ℝ) : Prop := ∀ u, α * ‖u‖ ≤ ‖a u‖

/-- The transposed nondegeneracy condition: every `v ≠ 0` is detected by some `u`, in the sense that
`a u v ≠ 0` ([han2009theoretical], (8.7.3)).  Equivalently, no nonzero `v` is orthogonal to the
range of the operator associated with `a`. -/
