...
functional needs to be convex: convexity enters only in the uniqueness clause
`IsMinOn.eq_of_strictConvexOn`.

The infinite-dimensional theorems of the same section — existence on a closed convex set of a
*reflexive* space — are not stated here.  They rest on weak sequential compactness of bounded
sets in a reflexive space, which Mathlib does not have; stating them with that compactness as an
...
/-- A functional is **coercive on a set** when it tends to `+∞` along the set as the norm grows:
for every level `M` there is a radius `R` beyond which `f ≥ M` on `K` (Atkinson–Han,
*Theoretical Numerical Analysis*, Def 3.3.9).

This is the condition on a *functional*, and is unrelated to the coercivity of an *operator* or
of a sesquilinear form, `re ⟪A x, x⟫ ≥ c ‖x‖²`, which is a lower bound of quadratic order rather
than a growth condition. -/
def IsCoerciveFunctionalOn (f : V → ℝ) (K : Set V) : Prop :=
  ∀ M : ℝ, ∃ R : ℝ, ∀ x ∈ K, R ≤ ‖x‖ → M ≤ f x

/-- A functional is coercive on every subset of a bounded set, vacuously: take `R` past the
bound. -/
...
/-- **Existence of a minimizer** (Atkinson–Han, *Theoretical Numerical Analysis*, Thm 3.3.13):
a lower semicontinuous functional on a nonempty closed subset `K` of a finite-dimensional
subspace `S` attains its minimum, provided `K` is bounded or `f` is coercive on `K`.

The two cases are one argument.  Coercivity confines the search to a sublevel set, closed by
lower semicontinuity and bounded by coercivity; a bounded `K` is coercive on itself vacuously
(`IsCoerciveFunctionalOn.of_isBounded`), so only the coercive case is proved.  Convexity of `K`
or of `f`, which the book assumes, is not used: it is what
`IsMinOn.eq_of_strictConvexOn` needs, not what existence needs. -/
...
functional on a nonempty closed bounded subset of a finite-dimensional subspace attains its
minimum. -/
...
/-- **Uniqueness of a minimizer** (the second clause of Atkinson–Han, *Theoretical Numerical
Analysis*, Thm 3.3.13, and of their Thm 3.3.12): a strictly convex functional has at most one
minimizer on a set.  This is Mathlib's `StrictConvexOn.eq_of_isMinOn` in the vocabulary of this
module, kept so that the module states the whole theorem. -/
