...
* Names: `SaadSparse.Chapter01.theorem_1_34`, `SaadSparse.Chapter05.proposition_5_3`, `equation_1_76`, …, plus descriptive
...
All of these are open nodes of the plan now: item 1 in `Numlib/LinearAlgebra/Matrix/Complexify.toml`,
2 in `Numlib/Analysis/Normed/Algebra/SpectralRadius.toml`, 3 in
`Numlib/LinearSolve/Stationary/RegularSplitting.toml` with `Numlib/LinearAlgebra/Matrix/Order.toml`,
4 in `Numlib/LinearSolve/Stationary/DiagDominant.toml`, 5 in
`Numlib/LinearSolve/Stationary/SPD.toml` and `ConsistentlyOrdered.toml`, 6 in
`Numlib/LinearSolve/Projection/Additive.toml` and `Numlib/LinearSolve/Stationary/Block.toml`, and
the candidates of item 7 in `Numlib/LinearSolve/Perturbation.toml` and
`Numlib/LinearSolve/Projection/OneDimensional.toml`. The surface results that wait on them
(`theorem_4_4`, `theorem_4_7`, `theorem_4_10`, `theorem_4_16`, `additiveStep_eq`, …) are open nodes
of the section groups.
