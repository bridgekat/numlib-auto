...
The plan of the library, in the TOML that [tracker](../tools/tracker/README.md) reads.
...
writes some three thousand public declarations; this plan names some fifteen hundred of them, by two
rules:
...
the proof passed through helpers on the way. In particular, not every dependency must be named.
...
A proved node carries nothing but its id: kind, description and dependencies are superseded by the
declaration, its doc comment and its proof. The open nodes are the plans for declarations that do
not exist yet — the phase 2–4 backbone modules (`Numlib/Krylov/Singular`,
`Numlib/LinearSolve/Stationary/RegularSplitting`, `Numlib/FloatingPoint/Model`, …), the
later-phase items appended to finished modules, and the book results each surface defers to
them — and they carry `kind`, `desc` and `deps` until they do. A `deps` list names tracked nodes
only, and only the key ones: it is a suggestion for `ready`, and the route a proof takes through
untracked helpers is read from the proof once it exists, so nothing is added to the plan merely
to be named in a `deps`.

## Shape

A group *is* a module, named by the module's path, so this directory is a copy of the source tree
with `.lean` replaced by `.toml`: `plans/Numlib/Krylov/CG.toml` is the plan for
...
directory is for. A group whose module is not written yet — the phase 2–4 modules — carries the
`desc` that the module's doc comment will replace; every finished group has a real module whose
`/-! … -/` doc comment describes it, and a plan copy would only be superseded. Both roots are of
that finished kind, so `Numlib.toml` and `NumlibSurface.toml` are empty files, kept because a
directory must have its group file beside it.
