...
* Naming: `AtkinsonHan.Chapter02.theorem_2_3_1`, `corollary_2_3_3`, `lemma_3_4_1`, `proposition_3_6_9_a`, `example_3_6_7`, `exercise_3_6_7`,
...
Items this chapter needs from later phases of `backbone.md` §7. They are open nodes of the plan
now — items 1–2 in `Numlib/Analysis/Normed/Operator/BanachSteinhaus.toml` and
`Numlib/Approximation/Quadrature.toml`, 3 in `Numlib/Approximation/Chebyshev.toml`, 4 in
`Numlib/Approximation/Interpolation.toml`, 5 in `Numlib/Krylov/OrthogonalPolynomials.toml`, 6 in
`Numlib/Approximation/Trigonometric.toml`, 7 in `Numlib/IntegralEquations/Basic.toml`, 9 in
`Numlib/Variational/Minimization.toml`, and 10 in `Numlib/Analysis/Normed/Module/WeakDual.toml`,
`Numlib/Analysis/Convex/StrictConvexSpace.toml` and `Numlib/Analysis/Normed/Ring/CondNumber.toml`;
item 8 (reflexive spaces) is deliberately not a node, for the reason given in the `Minimization`
group. The surface results that wait on them are open nodes of the section groups
(`theorem_3_3_19`, `theorem_3_7_1`, `equation_2_4_4`, …). The list below is kept as the record of
how each item was read off the book.
