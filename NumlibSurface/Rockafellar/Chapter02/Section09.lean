...
Surface file for R. Tyrrell Rockafellar, *Convex Analysis*, Princeton University Press, 1970, §9:
criteria, built on §8's recession cones, for an operation on convex sets or convex functions to
preserve closedness.

The section introduces no new concepts. All 18 numbered results of §9 are formalized. Where the
book states a result for `C₁, …, Cₘ` (or `f₁, …, fₘ`) — Corollaries 9.1.1, 9.1.3 and 9.2.1,
Theorem 9.3, and Theorem 9.8 with its corollaries — what is stated here is the `m = 2` instance.

## Main definitions

* `ExtCoeff` — Rockafellar's extended coefficient `λ ≥ 0⁺`: a real, or the formal symbol `0⁺`.
  `ExtCoeff.smulSet` and `ExtCoeff.smulFn` are its actions on sets and on functions, and
  `ExtCoeff.Nonneg`, `ExtCoeff.Pos` and `ExtCoeff.toReal` are the index conditions and the
  numerical value the book's `λ₁ + ⋯ + λₘ = 1` refers to.

## Main results

* `theorem_9_1_closure`, `theorem_9_1_recession`, `theorem_9_1_isClosed` — the image of a closed
  convex set under a linear map is closed when no non-zero recession direction is annihilated.
* `corollary_9_1_1_closure`, `corollary_9_1_1_recession`, `corollary_9_1_1_isClosed`,
  `corollary_9_1_2_isClosed`, `corollary_9_1_2_recession`,
  `corollary_9_1_2_isClosed_of_isBounded`, `corollary_9_1_3` — the same for a sum of sets, and for
  convex cones.
* `theorem_9_2`, `theorem_9_2_attained` — the image `Ah` of a closed proper convex function is
  closed, with the defining infimum attained.
* `corollary_9_2_1`, `corollary_9_2_2`, with their `_recession` and `_attained` clauses — the
  infimal convolute of closed proper convex functions is closed, and `(f₁ □ f₂)0⁺ = f₁0⁺ □ f₂0⁺`.
* `theorem_9_3_closed`, `theorem_9_3_recession`, `theorem_9_3_closure` — sums of functions.
* `theorem_9_4_closed`, `theorem_9_4_proper`, `theorem_9_4_recession`, `theorem_9_4_closure` —
  pointwise suprema.
* `theorem_9_5_closed`, `theorem_9_5_recession`, `theorem_9_5_closure` — composition `gA` with a
  linear transformation.
* `theorem_9_6`, `theorem_9_6_iUnion`, `corollary_9_6_1` — the closure of the convex cone
  generated by a closed convex set not containing the origin.
* `theorem_9_7`, `theorem_9_7_proper`, `theorem_9_7_attained`, `theorem_9_7_closed`,
  `theorem_9_7_iInf_pos` — the positively homogeneous convex function generated by `f`, and
  `corollary_9_7_1_closed`, `corollary_9_7_1_level`, `corollary_9_7_1_zero` for the gauge of a
  closed convex set containing the origin.
* `theorem_9_8`, `theorem_9_8_recession`, `corollary_9_8_1_isClosed`, `corollary_9_8_1_recession`,
  `corollary_9_8_2`, `corollary_9_8_3`, `corollary_9_8_3_recession`, `corollary_9_8_3_attained` —
  when the convex hull of a union is closed, and the corresponding statement for functions.
...
`z ∈ constancySpace h`". -/
theorem theorem_9_2 {f : Rn n → EReal} (hconv : ConvexFn f) (hp : ProperConvex f)
    (hc : IsClosed (epi f)) (A : Rn n →ₗ[ℝ] Rn m)
    (hrec : ∀ z, recessionFn f z ≤ 0 → A z = 0 → z ∈ constancySpace f) :
    ClosedProperConvexFn (mapLin A f) :=
  (closedProperConvexFn_mapLin (.of_isClosed_epi hconv hc hp) A hrec).2.1

/-- **Theorem 9.2**, last sentence: for each `y` with `(Ah)(y) ≠ +∞` the infimum defining
`(Ah)(y)` is attained. -/
theorem theorem_9_2_attained {f : Rn n → EReal} (hconv : ConvexFn f) (hp : ProperConvex f)
    (hc : IsClosed (epi f)) (A : Rn n →ₗ[ℝ] Rn m)
    (hrec : ∀ z, recessionFn f z ≤ 0 → A z = 0 → z ∈ constancySpace f)
    {y : Rn m} {μ : ℝ} (hμ : mapLin A f y ≤ (μ : EReal)) :
    ∃ x : Rn n, A x = y ∧ f x ≤ (μ : EReal) :=
  exists_mapLin_eq (.of_isClosed_epi hconv hc hp) A hrec hμ
...
theorem theorem_9_3_closed {f g : Rn n → EReal} (hf : ClosedProperConvexFn f)
    (hg : ClosedProperConvexFn g) (hne : (convexDom (f + g)).Nonempty) :
...
`cl (f₁ + f₂) = cl f₁ + cl f₂`. -/
theorem theorem_9_3_closure {f g : Rn n → EReal} (hf : ConvexFn f) (hpf : ProperConvex f)
    (hg : ConvexFn g) (hpg : ProperConvex g) {x : Rn n} (hxf : x ∈ ri (convexDom f))
        (hxg : x ∈ ri (convexDom g)) :
    convexCl (f + g) = convexCl f + convexCl g :=
  convexCl_add hf hpf hg hpg hxf hxg
...
everywhere. -/
theorem theorem_9_4_proper {ι : Type*} {f : ι → Rn n → EReal} (hp : ∀ i, ProperConvex (f i))
    {x : Rn n} (hbot : (⨆ i, f i x) ≠ ⊥) (htop : (⨆ i, f i x) ≠ ⊤) :
    ProperConvex (fun z => ⨆ i, f i z) := by
  have hι : Nonempty ι := by
    by_contra hne
    rw [not_nonempty_iff] at hne
    exact hbot (by simp)
  obtain ⟨i₀⟩ := hι
  refine ⟨⟨x, mem_convexDom.2 (lt_top_iff_ne_top.2 htop)⟩, fun z => ?_⟩
...
theorem theorem_9_4_closure {ι : Type*} {f : ι → Rn n → EReal} (hconv : ∀ i, ConvexFn (f i))
    {x : Rn n} (hx : ∀ i, x ∈ ri (convexDom (f i))) (hfin : (⨆ i, f i x) < ⊤) :
...
theorem theorem_9_5_recession {g : Rn m → EReal} (hg : ConvexFn g) (hc : IsClosed (epi g))
    (A : Rn n →ₗ[ℝ] Rn m) (hne : (convexDom (compLin g A)).Nonempty) :
...
`cl (gA) = (cl g)A`. -/
theorem theorem_9_5_closure {g : Rn m → EReal} (hg : ConvexFn g) (hp : ProperConvex g)
    (A : Rn n →ₗ[ℝ] Rn m) {x : Rn n} (hx : A x ∈ ri (convexDom g)) :
    convexCl (compLin g A) = compLin (convexCl g) A :=
  convexCl_compLin hg hp A hx
...
theorem theorem_9_7_proper {f : Rn n → EReal} (hf : ConvexFn f) (hp : ProperConvex f)
    (hc : IsClosed (epi f)) (h0 : 0 < f 0) :
    ProperConvex (posHomGen f) :=
  properConvex_posHomGen hf hp hc h0
...
theorem theorem_9_7 {f : Rn n → EReal} (hf : ConvexFn f) (hp : ProperConvex f)
    (hc : IsClosed (epi f))
    (h0 : 0 < f 0) :
...
theorem theorem_9_7_attained {f : Rn n → EReal} (hf : ConvexFn f) (hp : ProperConvex f)
...
theorem theorem_9_7_closed {f : Rn n → EReal} (hf : ConvexFn f) (hp : ProperConvex f)
...
theorem theorem_9_7_iInf_pos {f : Rn n → EReal} (hf : ConvexFn f) (hp : ProperConvex f)
    (hc : IsClosed (epi f)) (h0 : 0 < f 0) (hdom : f 0 ≠ ⊤) :
...
theorem corollary_9_7_1_closed {C : Set (Rn n)} (hC : Convex ℝ C) (h0 : (0 : Rn n) ∈ C)
  ...
  closedConvex_gaugeFn hC h0 hcl
