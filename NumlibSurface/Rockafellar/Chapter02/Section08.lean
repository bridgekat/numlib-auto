...
Surface file for R. Tyrrell Rockafellar, *Convex Analysis*, Princeton University Press, 1970, §8:
the directions in which an unbounded convex set recedes, and the recession function of a convex
function.

All 18 numbered results of §8 are formalized. The section's definitions are recorded alongside:
`0⁺C` (`recessionCone`), the lineality space and lineality of a set, `f0⁺` (`recessionFn`), and
...
* `theorem_8_1_cone`, `theorem_8_1_add_subset` — `0⁺C` is a convex cone containing the origin, and
  equals `{y | C + y ⊆ C}`.
* `theorem_8_2_isClosed`, `theorem_8_2_mem_iff`, `theorem_8_2_closure_cone` — for closed convex
  `C`, `0⁺C` is closed, is the set of limits `lim λᵢ xᵢ` with `λᵢ ↓ 0`, and is exactly what the
  closure of the cone `K` generated by `{(1, x) | x ∈ C}` adjoins:
  `cl K = K ∪ {(0, x) | x ∈ 0⁺C}`.
* `theorem_8_3`, `theorem_8_3_relint`, `corollary_8_3_1`, `corollary_8_3_2_iInter`,
  `corollary_8_3_3`, `corollary_8_3_4` — one half-line in a closed convex set already gives a
  recession direction, and the consequences for `ri C`, for intersections and for linear images.
* `theorem_8_4`, `corollary_8_4_1` — a non-empty closed convex set is bounded exactly when
  `0⁺C = {0}`; this is the one place in §8 where finite dimension is genuinely used.
* `linealitySubmodule_isGreatest_surface`, `linealitySpace_add_inter_orthogonal` — the lineality
  space is the largest subspace of `0⁺C`, and `C = L + (C ∩ L^⊥)`.
* `theorem_8_5_posHomogeneous`, `theorem_8_5_convex`, `theorem_8_5_proper`, `theorem_8_5_closed`,
  `theorem_8_5_sup`, `theorem_8_5_sup_quotient`, `theorem_8_5_lim` — `f0⁺` is a positively
  homogeneous convex function, closed and proper when `f` is, given by the difference formula, the
  difference-quotient supremum and the limit as `λ → ∞`.
* `corollary_8_5_1`, `corollary_8_5_2`, `corollary_8_5_2_zero_mem_dom` — `f0⁺` is the least `h`
  with `f z ≤ f x + h (z - x)`, and is the limit of `fλ` as `λ ↓ 0`.
* `theorem_8_6_antitone`, `theorem_8_6_iff`, `theorem_8_6_closed`, `corollary_8_6_1`,
  `corollary_8_6_2` — `f` is non-increasing along `y` exactly when `(f0⁺) y ≤ 0`, constant along
  `y` exactly when both `(f0⁺)(±y) ≤ 0`, and constant on an affine set where it is bounded above.
* `theorem_8_7_recessionCone`, `theorem_8_7_linealitySpace`, `corollary_8_7_1` — for closed proper
  convex `f` the non-empty level sets share a recession cone and a lineality space, so one bounded
  level set makes them all bounded.
* `theorem_8_8_a_iff_b`, `theorem_8_8_b_iff_c`, `theorem_8_8_a_iff_c`, `theorem_8_8_closed` — the
  directions along which `f` is affine, and the lineality space of `f` as a subspace.
