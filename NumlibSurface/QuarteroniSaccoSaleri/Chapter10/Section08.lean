...
-- TODO(backbone): a root of a continuous function on `[a, b]` strictly between two points where
-- it takes values of opposite strict signs; the same statement for `F : ℝ → ℝ` is private in
-- `Numlib/Approximation/Chebyshev` (`exists_root_of_mul_neg`), and this is where the alternation
-- vocabulary of that module would state it for `C(Icc a b, ℝ)`.
...
theorem exists_mem_Ioo_eq_zero_of_mul_neg {g : C(Icc a b, ℝ)} {u v : Icc a b}
...
theorem property_10_1_interpolates {f p : C(Icc a b, ℝ)}
  ...
  have hroot : ∀ k : Fin (n + 1), ∃ z : Icc a b,
    ...
    · -- `f = p`: the midpoint serves
      ...
      · rw [hfp]
        rfl
    · refine exists_mem_Ioo_eq_zero_of_mul_neg (hlt k) ?_
