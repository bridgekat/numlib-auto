...
zero second derivative vanishing at both ends vanishes — are marked `TODO(backbone)`.
...
/-! ### Calculus on a compact interval

The four lemmas of this section are general facts about real functions on `[a, b]` that Mathlib
does not state in this form; nothing in them is specific to the model problem. -/

-- TODO(backbone): `ContDiffOn ℝ 2` from explicit within-derivative data on a set of unique
-- differentiability; natural home beside `ContDiffMapIcc.contDiffOn` in
-- `Numlib/Analysis/Calculus/ContDiffMapIcc`.
...
-- TODO(backbone): the interior second derivative of a function given by within-derivative data
-- on `[a, b]`; same home as the previous lemma.
/-- If `u` has derivative `u₁` within `[a, b]` and `u₁` has derivative `u₂` within `[a, b]` at
every point of `[a, b]`, then at interior points `u'' = u₂`. -/
theorem iteratedDeriv_two_eq_of_hasDerivWithinAt {u u₁ u₂ : ℝ → ℝ} {a b : ℝ}
...
-- TODO(backbone): the interior derivatives of a `C²` function on `[a, b]`, in the form the
-- fundamental theorem of calculus consumes; same home as the previous lemma.
...
theorem hasDerivAt_of_contDiffOn_two {u : ℝ → ℝ} {a b : ℝ} (hu : ContDiffOn ℝ 2 u (Icc a b))
    (hx : x ∈ Ioo a b) :
...
-- TODO(backbone): a function on `[a, b]` whose derivative has zero derivative in the interior
-- and which vanishes at both ends vanishes identically; natural home beside
-- `constant_of_derivWithin_zero` in Mathlib's `Mathlib/Analysis/Calculus/MeanValue`.
