...
`v_{j+1}`, `Ch06.γ h β m` its `γ_{m+1}` and `Ch06.c h m` its `c_{m+1}`.
-/
...
theorem bcg_isPetrovGalerkin (h : BCGNoBreakdown A b x₀ rs₀ m) :
    IsPetrovGalerkin (op A) b x₀ (Ch06.krylov A (Ch06.r₀ A b x₀) m) (Ch06.krylov Aᴴ rs₀ m)
      (bcg A b x₀ rs₀ m).x := by
  simp only [Ch06.krylov_eq, bcg_eq]
...
theorem bcg_eq_lanczosSolve (h : BCGNoBreakdown A b x₀ rs₀ m)
    (hnb : NoBreakdown A (Ch06.v₁ A b x₀) (w₁ A b x₀ rs₀) m)
    (hT : IsUnit (T A (Ch06.v₁ A b x₀) (w₁ A b x₀ rs₀) m))
    (hr : Ch06.r₀ A b x₀ ≠ 0) (hw : inner 𝕜 (Ch06.v₁ A b x₀) rs₀ ≠ 0) :
    (bcg A b x₀ rs₀ m).x = lanczosSolve A b x₀ (w₁ A b x₀ rs₀) m := by
  have hK : Ch06.krylov A (Ch06.v₁ A b x₀) m = Ch06.krylov A (Ch06.r₀ A b x₀) m := by
    rw [Ch06.v₁_def,
      Ch06.krylov_smul A _ (inv_ne_zero (RCLike.ofReal_ne_zero.2 (norm_ne_zero_iff.2 hr)))]
  have hL : Ch06.krylov Aᴴ (w₁ A b x₀ rs₀) m = Ch06.krylov Aᴴ rs₀ m := by
    rw [show w₁ A b x₀ rs₀ = (inner 𝕜 (Ch06.v₁ A b x₀) rs₀)⁻¹ • rs₀ from rfl,
      Ch06.krylov_smul Aᴴ _ (inv_ne_zero hw)]
...
noncomputable abbrev qmrCoeff (A : Matrix (Fin n) (Fin n) 𝕜)
  ...
  bilanczosCoeff A (Ch06.v₁ A b x₀) w₁
...
noncomputable def qmr (m : ℕ) : EuclideanSpace 𝕜 (Fin n) :=
  Ch06.qgmres x₀ (qmrV A b x₀ w₁) (qmrCoeff A b x₀ w₁) (Ch06.β A b x₀ : 𝕜) m

/-- The starting relation `r_0 = β v_1` in the form the backbone consumes. -/
private theorem smul_qmrV_zero :
    b - op A x₀ = (Ch06.β A b x₀ : 𝕜) • qmrV A b x₀ w₁ 0 := (smul_v₁_eq_r₀ A b x₀).symm
...
private theorem quasiResidualNorm_eq (h : ℕ → ℕ → 𝕜) (β : 𝕜) (m : ℕ) :
    Ch06.quasiResidualNorm h β m = ‖Krylov.gamma h β m‖ := rfl

private theorem gamma_eq (h : ℕ → ℕ → 𝕜) (β : 𝕜) (m : ℕ) :
    Ch06.γ h β m = Krylov.gamma h β m := rfl

private theorem givensC_eq (h : ℕ → ℕ → 𝕜) (m : ℕ) :
    Ch06.c h m = Krylov.givensC h m := rfl

private theorem givensS_eq (h : ℕ → ℕ → 𝕜) (m : ℕ) :
    Ch06.s h m = Krylov.givensS h m := rfl

/-- `‖β‖ = ‖r_0‖₂`: the scalar `β` is the real number `‖r_0‖` seen in `𝕜`. -/
private theorem norm_beta : ‖(Ch06.β A b x₀ : 𝕜)‖ = ‖Ch06.r₀ A b x₀‖ := by
  rw [Ch06.β_eq_norm_r₀, RCLike.norm_ofReal, abs_of_nonneg (norm_nonneg _)]
...
@[simp] theorem qmr_zero : qmr A b x₀ w₁ 0 = x₀ := by
  rw [qmr, Ch06.qgmres, Ch06.toEuclideanLin_colMatrix_apply]
  simp
...
theorem qmr_isQuasiMinResIterate {m : ℕ} (hR : IsUnit (Ch06.R (qmrCoeff A b x₀ w₁) m)) :
    Krylov.IsQuasiMinResIterate (qmrV A b x₀ w₁) (qmrCoeff A b x₀ w₁) (Ch06.β A b x₀ : 𝕜) x₀ m
      (qmr A b x₀ w₁ m) :=
  Ch06.qgmres_isQuasiMinResIterate x₀ _ _ _
...
theorem equation_7_16 (h : NoSeriousBreakdown A (Ch06.v₁ A b x₀) w₁) (m : ℕ) (y : Fin m → 𝕜) :
    b - op A (x₀ + ∑ j, y j • qmrV A b x₀ w₁ (j : ℕ))
      = ∑ i, (Krylov.firstVec (Ch06.β A b x₀ : 𝕜) (m + 1)
          - (Tbar A (Ch06.v₁ A b x₀) w₁ m).mulVec y) i • qmrV A b x₀ w₁ (i : ℕ) := by
...
theorem equation_7_18 (m : ℕ) :
    Ch06.quasiResidualNorm (qmrCoeff A b x₀ w₁) (Ch06.β A b x₀ : 𝕜) m
        = (∏ k ∈ Finset.range m, ‖Ch06.s (qmrCoeff A b x₀ w₁) k‖) * ‖Ch06.r₀ A b x₀‖ ∧
      Ch06.quasiResidualNorm (qmrCoeff A b x₀ w₁) (Ch06.β A b x₀ : 𝕜) (m + 1)
        = ‖Ch06.s (qmrCoeff A b x₀ w₁) m‖
          * Ch06.quasiResidualNorm (qmrCoeff A b x₀ w₁) (Ch06.β A b x₀ : 𝕜) m := by
  refine ⟨?_, ?_⟩
...
theorem equation_7_19 (h : NoSeriousBreakdown A (Ch06.v₁ A b x₀) w₁) {m : ℕ}
    (hT : IsUnit (T A (Ch06.v₁ A b x₀) w₁ (m + 1))) :
    Ch06.γ (qmrCoeff A b x₀ w₁) (Ch06.β A b x₀ : 𝕜) (m + 1) • qmrV A b x₀ w₁ (m + 1)
      = Ch06.c (qmrCoeff A b x₀ w₁) m • (b - op A (lanczosSolve A b x₀ w₁ (m + 1))) := by
  have hy : (Krylov.hessenbergSqOf (qmrCoeff A b x₀ w₁) (m + 1)).mulVec
      (lanczosSolveY A b x₀ w₁ (m + 1)) = Krylov.firstVec (Ch06.β A b x₀ : 𝕜) (m + 1) := by
  ...
  have hres : b - op A (lanczosSolve A b x₀ w₁ (m + 1))
      = -(qmrCoeff A b x₀ w₁ (m + 1) m
        ...
        • qmrV A b x₀ w₁ (m + 1) := by
    rw [show qmrCoeff A b x₀ w₁ (m + 1) m = bilanczosDelta A (Ch06.v₁ A b x₀) w₁ (m + 1) from
...
theorem equation_7_20 (h : NoSeriousBreakdown A (Ch06.v₁ A b x₀) w₁)
...
private theorem rhoF_eq_norm_residual (hs : NoSeriousBreakdown A (Ch06.v₁ A b x₀) w₁)
    (hv : ∀ j, ‖qmrV A b x₀ w₁ j‖ = 1) (hT : ∀ k, IsUnit (T A (Ch06.v₁ A b x₀) w₁ k)) {m : ℕ}
    (h0 : Krylov.gamma (qmrCoeff A b x₀ w₁) (Ch06.β A b x₀ : 𝕜) m ≠ 0) {i : ℕ} (hi : i ≤ m) :
    rhoF (qmrCoeff A b x₀ w₁) (Ch06.β A b x₀ : 𝕜) i
      = ‖b - op A (lanczosSolve A b x₀ w₁ i)‖ := by
  cases i with
  | zero => rw [rhoF_zero, lanczosSolve_zero, norm_beta]
  | succ k =>
    have hk : Krylov.gamma (qmrCoeff A b x₀ w₁) (Ch06.β A b x₀ : 𝕜) k ≠ 0 := fun hcc =>
      h0 (gamma_eq_zero_of_le _ _ (by omega) hcc)
    have hy : (Krylov.hessenbergSqOf (qmrCoeff A b x₀ w₁) (k + 1)).mulVec
        (lanczosSolveY A b x₀ w₁ (k + 1)) = Krylov.firstVec (Ch06.β A b x₀ : 𝕜) (k + 1) := by
      rw [← T_eq_hessenbergSqOf]
...
private theorem gamma_ne_zero (hs : NoSeriousBreakdown A (Ch06.v₁ A b x₀) w₁)
    (hv : ∀ j, ‖qmrV A b x₀ w₁ j‖ = 1) (hT : ∀ k, IsUnit (T A (Ch06.v₁ A b x₀) w₁ k))
    (hne : ∀ j, b - op A (lanczosSolve A b x₀ w₁ j) ≠ 0) (m : ℕ) :
    Krylov.gamma (qmrCoeff A b x₀ w₁) (Ch06.β A b x₀ : 𝕜) m ≠ 0 := by
  induction m with
  | zero =>
    intro hcc
    have hb : (Ch06.β A b x₀ : 𝕜) = 0 := hcc
    have hnb : ‖Ch06.r₀ A b x₀‖ = 0 := by
...
theorem proposition_7_3 (h : NoSeriousBreakdown A (Ch06.v₁ A b x₀) w₁) {m : ℕ}
    (hR : IsUnit (Ch06.R (qmrCoeff A b x₀ w₁) m)) {C : ℝ}
    (hC : ∀ w : Fin (m + 1) → 𝕜, ‖∑ i, w i • qmrV A b x₀ w₁ (i : ℕ)‖
      ≤ C * ‖(WithLp.toLp 2 w : EuclideanSpace 𝕜 (Fin (m + 1)))‖) :
    ‖b - op A (qmr A b x₀ w₁ m)‖
      ≤ C * ((∏ k ∈ Finset.range m, ‖Ch06.s (qmrCoeff A b x₀ w₁) k‖) * ‖Ch06.r₀ A b x₀‖) := by
  ...
  exact Krylov.IsQuasiMinResIterate.norm_residual_le
    ...
    ((Ch06.isUnit_R_iff _ (fun _ _ hij => bilanczosCoeff_eq_zero_of_lt A _ _ hij)).1 hR) hC
...
theorem proposition_7_3_sqrt (h : NoSeriousBreakdown A (Ch06.v₁ A b x₀) w₁) {m : ℕ}
    (hR : IsUnit (Ch06.R (qmrCoeff A b x₀ w₁) m)) (hv : ∀ j, ‖qmrV A b x₀ w₁ j‖ ≤ 1) :
    ‖b - op A (qmr A b x₀ w₁ m)‖
      ≤ Real.sqrt (m + 1)
        * ((∏ k ∈ Finset.range m, ‖Ch06.s (qmrCoeff A b x₀ w₁) k‖) * ‖Ch06.r₀ A b x₀‖) := by
...
theorem theorem_7_4 (h : NoSeriousBreakdown A (Ch06.v₁ A b x₀) w₁) {m : ℕ}
    (hnb : NoBreakdown A (Ch06.v₁ A b x₀) w₁ m) (hr : Ch06.r₀ A b x₀ ≠ 0)
    (hR : IsUnit (Ch06.R (qmrCoeff A b x₀ w₁) m)) (hm : m ≤ Ch06.grade A (Ch06.v₁ A b x₀))
    (hRA : IsUnit (Ch06.R (Ch06.arnoldiCoeff A (Ch06.v₁ A b x₀)) m)) {c C : ℝ} (hc0 : 0 < c)
    (hc : ∀ w : Fin (m + 1) → 𝕜, c * ‖(WithLp.toLp 2 w : EuclideanSpace 𝕜 (Fin (m + 1)))‖
      ≤ ‖∑ i, w i • qmrV A b x₀ w₁ (i : ℕ)‖)
    (hC : ∀ w : Fin (m + 1) → 𝕜, ‖∑ i, w i • qmrV A b x₀ w₁ (i : ℕ)‖
      ≤ C * ‖(WithLp.toLp 2 w : EuclideanSpace 𝕜 (Fin (m + 1)))‖) :
    ‖b - op A (qmr A b x₀ w₁ m)‖ ≤ C / c * ‖b - op A (Ch06.gmresFixed A b x₀ m)‖ := by
  have hspan : Submodule.span 𝕜 (Set.range fun i : Fin m => qmrV A b x₀ w₁ (i : ℕ))
      = Ch06.krylov A (Ch06.r₀ A b x₀) m := by
    rw [(proposition_7_1_span hnb).1, Ch06.v₁_def,
      Ch06.krylov_smul A _ (inv_ne_zero (RCLike.ofReal_ne_zero.2 (norm_ne_zero_iff.2 hr)))]
  exact Ch06.theorem_6_11 b x₀ (bilanczos_hessenbergRelation h) (smul_qmrV_zero A b x₀ w₁) hR hm
    hRA hspan hc0 hc hC
...
(6.65). -/
theorem equation_7_23 (h : NoSeriousBreakdown A (Ch06.v₁ A b x₀) w₁)
    (hv : ∀ j, ‖qmrV A b x₀ w₁ j‖ = 1) {m : ℕ}
    (hT : IsUnit (T A (Ch06.v₁ A b x₀) w₁ (m + 1)))
    (h0 : Ch06.γ (qmrCoeff A b x₀ w₁) (Ch06.β A b x₀ : 𝕜) (m + 1) ≠ 0) :
    1 / Ch06.quasiResidualNorm (qmrCoeff A b x₀ w₁) (Ch06.β A b x₀ : 𝕜) (m + 1) ^ 2
      = 1 / Ch06.quasiResidualNorm (qmrCoeff A b x₀ w₁) (Ch06.β A b x₀ : 𝕜) m ^ 2
        + 1 / ‖b - op A (lanczosSolve A b x₀ w₁ (m + 1))‖ ^ 2 := by
  have hk : Krylov.gamma (qmrCoeff A b x₀ w₁) (Ch06.β A b x₀ : 𝕜) m ≠ 0 := fun hcc =>
  ...
  rw [quasiResidualNorm_eq, givensC_eq] at h20
  have hkey : ‖Krylov.gamma (qmrCoeff A b x₀ w₁) (Ch06.β A b x₀ : 𝕜) (m + 1)‖
...
theorem equation_7_24 (h : NoSeriousBreakdown A (Ch06.v₁ A b x₀) w₁)
    (hv : ∀ j, ‖qmrV A b x₀ w₁ j‖ = 1)
    (hT : ∀ k, IsUnit (T A (Ch06.v₁ A b x₀) w₁ k)) {m : ℕ}
    (h0 : Ch06.γ (qmrCoeff A b x₀ w₁) (Ch06.β A b x₀ : 𝕜) m ≠ 0) :
    1 / Ch06.quasiResidualNorm (qmrCoeff A b x₀ w₁) (Ch06.β A b x₀ : 𝕜) m ^ 2
...
theorem proposition_7_5 (h : NoSeriousBreakdown A (Ch06.v₁ A b x₀) w₁)
    (hv : ∀ j, ‖qmrV A b x₀ w₁ j‖ = 1)
    (hT : ∀ k, IsUnit (T A (Ch06.v₁ A b x₀) w₁ k)) {m : ℕ}
    (h0 : Ch06.γ (qmrCoeff A b x₀ w₁) (Ch06.β A b x₀ : 𝕜) m ≠ 0)
  ...
  · obtain ⟨i, hi, hle⟩ := Krylov.exists_norm_gamma_div_givensC_le (qmrCoeff A b x₀ w₁)
      (Ch06.β A b x₀ : 𝕜) (rhoF_zero _ _) (rhoF_succ _ _) m
...
theorem equation_7_26 (h : NoSeriousBreakdown A (Ch06.v₁ A b x₀) w₁) {m : ℕ}
    (hR : IsUnit (Ch06.R (qmrCoeff A b x₀ w₁) m)) :
    b - op A (qmr A b x₀ w₁ m)
        = Ch06.γ (qmrCoeff A b x₀ w₁) (Ch06.β A b x₀ : 𝕜) m
          • Ch06.z (qmrV A b x₀ w₁) (qmrCoeff A b x₀ w₁) m ∧
      Ch06.z (qmrV A b x₀ w₁) (qmrCoeff A b x₀ w₁) (m + 1)
        = starRingEnd 𝕜 (-Ch06.s (qmrCoeff A b x₀ w₁) m)
            • Ch06.z (qmrV A b x₀ w₁) (qmrCoeff A b x₀ w₁) m
          + starRingEnd 𝕜 (Ch06.c (qmrCoeff A b x₀ w₁) m) • qmrV A b x₀ w₁ (m + 1) := by
  exact ⟨Ch06.equation_6_50 (bilanczos_hessenbergRelation h) (smul_qmrV_zero A b x₀ w₁) hR,
    Ch06.equation_6_53 _ _ m⟩
...
theorem equation_7_29 {m : ℕ}
    ...
    (hRm : IsUnit (Ch06.R (qmrCoeff A b x₀ w₁) m))
    (hR : IsUnit (Ch06.R (qmrCoeff A b x₀ w₁) (m + 1)))
    (hT : IsUnit (T A (Ch06.v₁ A b x₀) w₁ (m + 1))) :
    qmr A b x₀ w₁ (m + 1)
      = ((‖Ch06.s (qmrCoeff A b x₀ w₁) m‖ ^ 2 : ℝ) : 𝕜) • qmr A b x₀ w₁ m
        + ((‖Ch06.c (qmrCoeff A b x₀ w₁) m‖ ^ 2 : ℝ) : 𝕜)
...
private theorem qmrsTau_eq (hs : NoSeriousBreakdown A (Ch06.v₁ A b x₀) w₁)
    (hv : ∀ j, ‖qmrV A b x₀ w₁ j‖ = 1) (hT : ∀ k, IsUnit (T A (Ch06.v₁ A b x₀) w₁ k))
    (hne : ∀ j, b - op A (lanczosSolve A b x₀ w₁ j) ≠ 0) (m : ℕ) :
    Ch06.qmrsTau (fun j => b - op A (lanczosSolve A b x₀ w₁ j)) m
      = Ch06.quasiResidualNorm (qmrCoeff A b x₀ w₁) (Ch06.β A b x₀ : ℝ) m := by
  induction m with
  | zero =>
    have habs : ‖(Ch06.β A b x₀ : ℝ)‖ = ‖Ch06.r₀ A b x₀‖ :=
      (Real.norm_eq_abs (Ch06.β A b x₀)).trans (abs_of_nonneg (norm_nonneg (Ch06.r₀ A b x₀)))
...
private theorem qmrsEta_eq (hs : NoSeriousBreakdown A (Ch06.v₁ A b x₀) w₁)
    (hv : ∀ j, ‖qmrV A b x₀ w₁ j‖ = 1) (hT : ∀ k, IsUnit (T A (Ch06.v₁ A b x₀) w₁ k))
    (hne : ∀ j, b - op A (lanczosSolve A b x₀ w₁ j) ≠ 0) (m : ℕ) :
    Ch06.qmrsEta (fun j => b - op A (lanczosSolve A b x₀ w₁ j)) m
      = ‖Ch06.c (qmrCoeff A b x₀ w₁) m‖ ^ 2 := by
...
theorem qmrSmoothing_eq_qmr {A : Matrix (Fin n) (Fin n) ℝ}
    {b x₀ w₁ : EuclideanSpace ℝ (Fin n)} (h : NoSeriousBreakdown A (Ch06.v₁ A b x₀) w₁)
    (hv : ∀ j, ‖qmrV A b x₀ w₁ j‖ = 1)
    (hT : ∀ k, IsUnit (T A (Ch06.v₁ A b x₀) w₁ k))
    (hR : ∀ k, IsUnit (Ch06.R (qmrCoeff A b x₀ w₁) k))
    (hne : ∀ j, b - op A (lanczosSolve A b x₀ w₁ j) ≠ 0) (m : ℕ) :
    (qmrSmoothing (fun j => lanczosSolve A b x₀ w₁ j)
        (fun j => b - op A (lanczosSolve A b x₀ w₁ j)) m).1 = qmr A b x₀ w₁ m := by
  simp only [qmrSmoothing]
  induction m with
  | zero => simp [Ch06.qmrs_zero]
  | succ k ih =>
    have hρ := (Ch06.isUnit_R_iff (qmrCoeff A b x₀ w₁)
    ...
    rw [Ch06.qmrs_succ, ih, qmrsEta_eq h hv hT hne k,
