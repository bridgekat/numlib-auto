...
Section 10.2 of Yousef Saad, *Iterative Methods for Sparse Linear Systems*, 2nd edition,
SIAM, 2003: the classical stationary iterations of Chapter 4 read as preconditioners.
...
The letters `D`, `E`, `F` are the ones fixed by §4.1, in `SaadSparse.Ch04`.
-/
...
theorem equation_10_5 (h : IsUnit (diagPart A)) :
    (jacobiSplitting A h).iterationOperator = 1 - Ring.inverse (Ch04.D A) * A := rfl
...
theorem equation_10_6 (h : IsUnit (diagPart A)) :
    (gaussSeidelSplitting A h).iterationOperator =
      1 - Ring.inverse (Ch04.D A - Ch04.E A) * A := by
  rw [Ch04.D_sub_E]
  rfl
...
noncomputable def M_sgs (A : Matrix (Fin n) (Fin n) ℝ) : Matrix (Fin n) (Fin n) ℝ :=
  (Ch04.D A - Ch04.E A) * (Ch04.D A)⁻¹ * (Ch04.D A - Ch04.F A)
...
theorem M_sgs_eq_ssorSplitting (h : IsUnit (diagPart A)) :
  ...
  rw [M_sgs, Ch04.D_sub_E, Ch04.D_sub_F, Ch04.D]
  norm_num
...
def U_sgs (A : Matrix (Fin n) (Fin n) ℝ) : Matrix (Fin n) (Fin n) ℝ := Ch04.D A - Ch04.F A
...
theorem inv_D_eq (h : IsUnit (diagPart A)) :
    (Ch04.D A)⁻¹ = Matrix.diagonal fun i => (A i i)⁻¹ := by
  have hne : ∀ i, A i i ≠ 0 := (Matrix.isUnit_diagPart_iff A).mp h
  refine Matrix.inv_eq_right_inv ?_
  rw [Ch04.D, Matrix.diagPart, Matrix.diagonal_mul_diagonal, ← Matrix.diagonal_one]
  congr 1
  funext i
  exact mul_inv_cancel₀ (hne i)

theorem inv_D_apply (h : IsUnit (diagPart A)) (i j : Fin n) :
    (Ch04.D A)⁻¹ i j = if i = j then (A i i)⁻¹ else 0 := by
...
theorem L_sgs_apply (h : IsUnit (diagPart A)) (i j : Fin n) :
  ...
  have hprod : (strictLower A * (Ch04.D A)⁻¹) i j = strictLower A i j * (A j j)⁻¹ := by
  ...
  rw [L_sgs, Ch04.E, Matrix.sub_apply, Matrix.one_apply, Matrix.neg_mul, Matrix.neg_apply, hprod,
    strictLower_apply]
...
theorem U_sgs_apply (A : Matrix (Fin n) (Fin n) ℝ) (i j : Fin n) :
  ...
  rw [U_sgs, Ch04.D, Ch04.F, sub_neg_eq_add, Matrix.add_apply, diagPart_apply, strictUpper_apply]
...
the corresponding triangular part of `A` (`SaadSparse.Ch10.L_sgs_apply`,
`SaadSparse.Ch10.U_sgs_apply`). So a symmetric Gauss–Seidel sweep is an exact triangular
factorization of *some* matrix with the sparsity of `A`. -/
theorem M_sgs_eq_mul (h : IsUnit (diagPart A)) : M_sgs A = L_sgs A * U_sgs A := by
  have hdet : IsUnit (Ch04.D A).det := (isUnit_iff_isUnit_det _).mp h
...
theorem sub_M_sgs_eq (h : IsUnit (diagPart A)) :
    A - L_sgs A * U_sgs A = -(Ch04.E A * (Ch04.D A)⁻¹ * Ch04.F A) := by
  have hdet : IsUnit (Ch04.D A).det := (isUnit_iff_isUnit_det _).mp h
  have hA : A = Ch04.D A - Ch04.E A - Ch04.F A := Ch04.decomp A
  have hDD : Ch04.E A * (Ch04.D A)⁻¹ * Ch04.D A = Ch04.E A := by
