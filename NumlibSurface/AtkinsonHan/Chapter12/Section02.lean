...
* `piecewiseLinearLp` and `piecewiseLinearProjCLM` — the piecewise linear trial space inside
  `L²(a, b)` and the orthogonal projection onto it, with `norm_iccToLp_le` and
  `norm_sub_piecewiseLinearProjCLM_le` the two inequalities of (12.2.18) and
  `tendsto_piecewiseLinearProjCLM` the pointwise convergence on the whole of `L²`.
* `equation_12_2_19` — the piecewise linear Galerkin method: `‖K - P_n K‖ → 0`, unique solvability
  for large `n`, and `‖u - u_n‖_{L²} ≤ |λ| M √(b - a) h_n² ‖u''‖_∞ / 8` for a `C²` solution.
...
noncomputable abbrev iccToLp (a b : ℝ) :
    C(Set.Icc a b, ℝ) →L[ℝ] Lp ℝ 2 (IntegralOperator.iccMeasure a b) :=
  ContinuousMap.toLp (E := ℝ) 2 (IntegralOperator.iccMeasure a b) ℝ

/-- The total mass of Lebesgue measure on `[a, b]` is `b - a`. -/
theorem measureReal_iccMeasure_univ (hab : a ≤ b) :
    (IntegralOperator.iccMeasure a b).real Set.univ = b - a := by
  have h := IntegralOperator.integral_iccMeasure hab fun _ => (1 : ℝ)
  rw [integral_const, smul_eq_mul, mul_one] at h
  rw [h]
  simp

/-- **A uniform bound is an `L²` bound**: `‖f‖_{L²(a, b)} ≤ √(b - a) ‖f‖_∞`, the first inequality
of (12.2.18). -/
theorem norm_iccToLp_le (hab : a ≤ b) (f : C(Set.Icc a b, ℝ)) :
    ‖iccToLp a b f‖ ≤ √(b - a) * ‖f‖ := by
  have hb : ∀ᵐ y ∂(IntegralOperator.iccMeasure a b), ‖(iccToLp a b f) y‖ ≤ ‖f‖ := by
    filter_upwards [ContinuousMap.coeFn_toLp (E := ℝ) (p := 2)
      (μ := IntegralOperator.iccMeasure a b) (𝕜 := ℝ) f] with y hy
    rw [hy]
    exact f.norm_coe_le_norm y
  refine (Lp.norm_le_of_ae_bound (norm_nonneg f) hb).trans (le_of_eq ?_)
  have hmu : ((measureUnivNNReal (IntegralOperator.iccMeasure a b) : NNReal) : ℝ) = b - a := by
    rw [show ((measureUnivNNReal (IntegralOperator.iccMeasure a b) : NNReal) : ℝ)
      = (IntegralOperator.iccMeasure a b).real Set.univ from rfl]
...
/-- **An orthogonal projection has norm at most one**, which is where §12.2.3 is cheaper than
§12.2.1: no Lebesgue constant enters. -/
...
/-- **(12.2.18), the approximation property of the Galerkin projection**: the orthogonal projection
is at least as good as the interpolant, and a uniform bound is an `L²` bound, so
...
/-- **The Galerkin projections converge pointwise on the whole of `L²(a, b)`**, the paragraph of
§12.2.3 that follows (12.2.18): they do so on the continuous functions by (12.2.18) and the
convergence of piecewise linear interpolation, they have norm one, and the continuous functions are
dense in `L²`. -/
